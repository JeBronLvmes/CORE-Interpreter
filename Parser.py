# Author: Jeb Alawi
# Description: Parse Tree to parse and print CORE programs
# Date: 10/31/18
...
class ParseTree:
    ...
    # checks token to see if the next token begins a statement
    def isStmt(self):
        stmtOps = ["if", "while", "read", "write"]
        if self.tok.getTokenList()[-1] == 32 or self.tok.currentToken() in stmtOps:
            return True
        else:
            return False
    ...
    # parses condition statements into tree
    def parseCond(self):
    ...
    # parses comparasion term into tree
    def parseComp(self):
    ...
    # if current token is a compOp then token is consumed and function returns true
    ...
    def printStmtSeq(self, indent = 4):
        self.moveToChild(0)
        self.printStmt(indent)
        self.moveToParent()
        if(self.root.alt == 1):
            self.moveToChild(1)
            self.printStmtSeq(indent)
            self.moveToParent()
